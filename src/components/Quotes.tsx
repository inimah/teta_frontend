...
const Quotes: React.FC = () => {
  ...
  const moodColors: Record<string, string> = {
    ...
    marah: "bg-red-50",
    cemas: "bg-purple-50",
    motivasi: "bg-yellow-50",
    insecure: "bg-green-50",
    "self-love": "bg-pink-50",
    lucu: "bg-orange-50",
    bersyukur: "bg-teal-50",
    senang: "bg-indigo-50",
  };
  ...
  const moodTextColors: Record<string, string> = {
    sedih: "text-blue-800",
    marah: "text-red-800",
    cemas: "text-purple-800",
    motivasi: "text-yellow-800",
    insecure: "text-green-800",
    "self-love": "text-pink-800",
    lucu: "text-orange-800",
    bersyukur: "text-teal-800",
    senang: "text-indigo-800",
  };

  const cardTextColor = selectedMood ? moodTextColors[selectedMood.category_name] : cardTextColors[currentIndex % cardTextColors.length];
  const cardColor = selectedMood ? moodColors[selectedMood.category_name] : cardColors[currentIndex % cardColors.length];
...
const moods = [
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(import.meta.env.VITE_API_URL + `api/user-favs/${userId}`);
  ...
  const handleFavorite = async (quoteId: string) => {
    ...
    try {
      const res = await fetch(import.meta.env.VITE_API_URL + "api/user-favs", {
        method: "POST",
